...
const createNewItem = asyncHandler(async (req, res) => {
  ...
  const duplicate = await Item.findOne({ name, unitMeasure })
...
const updateItemTexts = asyncHandler(async (req, res) => {
  ...
  if (firstName !== name) {
    await fs.promises.rename(
      path.join(__dirname, "..", "public", "images", firstName),
      path.join(__dirname, "..", "public", "images", name),
    );
  }
...
const deleteItem = asyncHandler(async (req, res) => {
  ...
  if (
    fs.existsSync(path.join(__dirname, "..", "public", "images", `./${name}`))
  ) {
    ...
    if (data.length) {
      data.map(async (file) => {
        await fs.promises.unlink(
          path.join(__dirname, "..", "public", "images", name, file),
        );
      });
    }
