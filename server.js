...
app.delete("/delete-pic/:initialPic", async (req, res) => {
  ...
  try {
    ...
    await Item.findOneAndUpdate({ _id: id }, { img: currentPics });
    const data = await fs.promises.readdir(
      path.join(__dirname, "public", "images", name),
    );
    console.log({ content: data.length });
    if (
      fs.existsSync(path.join(__dirname, "public", "images", name, initialPic))
    ) {
      await fs.promises.unlink(
        path.join(__dirname, "public", "images", name, initialPic),
      );
